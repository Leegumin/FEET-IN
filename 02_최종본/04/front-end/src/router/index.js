  ...
  // * navbar + footer 있는 것
  {
    ...
    children : [
      ...
      },
      {
        ...
        component: () => import('@/components/category/CategoryDetailbackup'),
      },
      {
        path     : '/review',
        name     : 'review',
...
router.beforeEach((to, from, next) => {
  ...
  const authRequired = ['mypage', 'add-faq', 'edit-product', 'admin-page', 'add-product', 'edit-product', 'add-review', 'order', 'cart-order']
  // 인증 없이 접근가능한 경로 (화면)
  const publicPages = !authRequired.includes(to.name)
  // 웹 토큰을 가져와서 확인
