...
<!--    </div>-->
    ...
    <div class="card-wrapper" v-for="(board, index) in boards" :key="index">
      <div class="card mt-1">
       ...
       <br>
              <a class="review-content-title" @click="toogleShow(index)">
                {{ board.title }}
              </a>
              <div class="review" v-if="!board.type">
                {{ board.content }}
              </div>
              <div>{{ board.insertTime }}</div>
                              <button class="btn text-black" @click="deleteBoard(board.reviewId)" style="float: right">
                삭제
              </button>
            </div>
          </div>
